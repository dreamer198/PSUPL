...
struct DLL_LINKAGE Point {
	...
	Point(const Vec3f& Pos, size_t Tag) {/*index = -1;*/ pos = Pos; normal = Vec3f(0, 0, 0); tag = Tag; }
	Point(const Vec3f &Pos, const Vec3f &Normal) { pos = Pos; normal = Normal; }
	Point(const Vec3f& Pos, const Vec3f& Normal, size_t Tag) { pos = Pos; normal = Normal; tag = Tag; }
...
public:
	...
	void calcNormals(unsigned int kNN = 20, unsigned int maxTries = 100 );
