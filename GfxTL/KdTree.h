		...
		>
	{
		public:
			...
			void Build()
			{
				...
				BaseType::Clear(); // 清空八叉树
				if (!BaseType::size()) // 如果八叉树为空，则直接返回
					return;
				BaseType::Root() = new CellType; // 创建根节点
				std::deque< Pair > stack(1); // 初始化栈，存储待处理的节点及其构建信息
				stack.back().first = BaseType::Root(); // 根节点入栈
				InitRootBuildInformation(&stack.back().second); // 初始化根节点的构建信息
				BaseType::InitRoot(stack.back().second, BaseType::Root()); // 初始化根节点
				while (stack.size()) // 当栈不为空时，继续处理
				{
					Pair& p = stack.back(); // 获取栈顶元素
					if (p.second.CreateChild() == CellType::NChildren) // 如果当前节点的所有子节点已创建，则处理下一个节点
					{
						BaseType::LeaveGlobalBuildInformation(*p.first, p.second); // 保存当前节点的全局构建信息
						stack.pop_back(); // 弹出当前节点
						continue;
					}
					if (BaseType::IsLeaf(*p.first)) // 如果当前节点是叶子节点
					{
						if (!BaseType::ShouldSubdivide(*p.first, p.second)) // 如果不需要细分，则处理下一个节点
						{
							stack.pop_back(); // 弹出当前节点
							continue;
						}
						Subdivide(p.second, p.first); // 细分当前节点
						if (BaseType::IsLeaf(*p.first)) // 如果细分失败，则处理下一个节点
						{
							stack.pop_back(); // 弹出当前节点
							continue;
						}
					...
					while (p.second.CreateChild() < CellType::NChildren &&
						!ExistChild(*p.first, p.second.CreateChild())) // 查找下一个未创建的子节点
						++p.second.CreateChild();
					if (p.second.CreateChild() == CellType::NChildren) // 如果所有子节点都已创建，则处理下一个节点
					{
						stack.pop_back(); // 弹出当前节点
						continue;
					}
					BaseType::EnterGlobalBuildInformation(*p.first, &p.second); // 更新全局构建信息
					stack.resize(stack.size() + 1); // 创建新的栈元素
					stack.back().first = &(*p.first)[p.second.CreateChild()]; // 设置新的子节点
					InitBuildInformation(*p.first, p.second,
						p.second.CreateChild(), &stack.back().second); // 初始化子节点的构建信息
					InitCell(*p.first, p.second, p.second.CreateChild(),
						stack.back().second, &(*p.first)[p.second.CreateChild()]); // 初始化子节点
					++p.second.CreateChild(); // 继续处理下一个子节点
				}
			}
