...
class DLL_LINKAGE RansacShapeDetector
{
	public:
		...
		size_t Detect(PointCloud &pc,LineCloud &Lc, size_t begin, size_t end,
	...
	private:
		...
		float CandidateFailureProbability(float candidateSize, float numberOfPoints, float drawnCandidates, float levels, MiscLib::Vector< double > samplePlanProbability) const
		{
			return std::min(std::pow(1.f - candidateSize / (numberOfPoints * levels * (1 << 2)), drawnCandidates * float(samplePlanProbability[0])), 1.f)
					* std::min(std::pow(1.f - candidateSize / (numberOfPoints * levels * (1 << 1)), drawnCandidates * float(1 - samplePlanProbability[0])), 1.f);
		}
