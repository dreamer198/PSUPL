...
void RansacShapeDetector::GenerateCandidates(
	...
	const MiscLib::Vector< double >& samplePlanProbSum,
	size_t* drawnCandidates,
	MiscLib::Vector< std::pair< float, size_t > >* sampleLevelScores,
	MiscLib::Vector< std::pair< float, size_t > >* samplePlanScores,
		...
		for (int candIter = 0; candIter < 200; ++candIter)
		{
			...
			if (!isER) {
				if (INPUT_TYPE == 2) {
					...
					for (; samplePlan < samplePlanProbSum.size() - 1; ++samplePlan)
						if (samplePlanProbSum[samplePlan] >= p)
							break;
				}
			...
			if (!DrawSamplesStratified(globalOctree, sampleLevel,
				scoreVisitorCopy.GetShapeIndex(), &samples, &node, samplePlan)) {
				continue;
			}

			//std::string testFileName = "./output/test.obj";
			//std::ofstream testFile(testFileName);
			//std::string nodeFileName = "./output/node.obj";
			//std::ofstream nodeFile(nodeFileName);
			//if (samplePlan == 0) {
			//	for (int i = 0; i < 3; i++) {
			//		testFile << "v " << globalOctree.at(samples[i]).pos[0] << " " << globalOctree.at(samples[i]).pos[1] << " " << globalOctree.at(samples[i]).pos[2]
			//			<< " " << 0 << " " << 0 << " " << 1 << std::endl;
			//	}
			//}
			//else if (samplePlan == 1) {
			//	testFile << "v " << globalOctree.at(samples[0]).pos[0] << " " << globalOctree.at(samples[0]).pos[1] << " " << globalOctree.at(samples[0]).pos[2]
			//		<< " " << 0 << " " << 0 << " " << 1 << std::endl;
			//	testFile << "v " << Lc[samples[1]].startPoint[0] << " " << Lc[samples[1]].startPoint[1] << " " << Lc[samples[1]].startPoint[2]
			//		<< " " << 0 << " " << 0 << " " << 1 << std::endl;
			//	testFile << "v " << Lc[samples[1]].endPoint[0] << " " << Lc[samples[1]].endPoint[1] << " " << Lc[samples[1]].endPoint[2]
			//		<< " " << 0 << " " << 0 << " " << 1 << std::endl;
			//	testFile << "l " << 2 << " " << 3 << std::endl;
			//}
			//else {
			//	testFile << "v " << Lc[samples[0]].startPoint[0] << " " << Lc[samples[0]].startPoint[1] << " " << Lc[samples[0]].startPoint[2]
			//		<< " " << 0 << " " << 0 << " " << 1 << std::endl;
			//	testFile << "v " << Lc[samples[0]].endPoint[0] << " " << Lc[samples[0]].endPoint[1] << " " << Lc[samples[0]].endPoint[2]
			//		<< " " << 0 << " " << 0 << " " << 1 << std::endl;
			//	testFile << "v " << Lc[samples[1]].startPoint[0] << " " << Lc[samples[1]].startPoint[1] << " " << Lc[samples[1]].startPoint[2]
			//		<< " " << 0 << " " << 0 << " " << 1 << std::endl;
			//	testFile << "v " << Lc[samples[1]].endPoint[0] << " " << Lc[samples[1]].endPoint[1] << " " << Lc[samples[1]].endPoint[2]
			//		<< " " << 0 << " " << 0 << " " << 1 << std::endl;
			//	testFile << "l " << 1 << " " << 2 << std::endl;
			//	testFile << "l " << 3 << " " << 4 << std::endl;
			//}
			//for (int i = 0; i < (*node).pointIdx.size(); i++) {
			//	nodeFile << "v " << globalOctree.at((*node).pointIdx[i]).pos[0] << " " << globalOctree.at((*node).pointIdx[i]).pos[1] << " " << globalOctree.at((*node).pointIdx[i]).pos[2]
			//		<< " " << 1 << " " << 0 << " " << 0 << std::endl;
			//}
			//testFile.close();
			//nodeFile.close();
			...
			ConstructorsType::const_iterator planeShapeIter = m_constructors.begin();

			MiscLib::Vector< Vec3f > samplePoints(3 << 1);
			if (samplePlan == 0/* || samplePlan == 2*/) {
				for (size_t i = 0; i < 3; ++i)
				{
					samplePoints[i] = globalOctree.at(samples[i]).pos;
					samplePoints[i + 3] = globalOctree.at(samples[i]).normal;
				}
			}
			else if (samplePlan == 1) {
				samplePoints[0] = globalOctree.at(samples[0]).pos;
				samplePoints[1] = Lc[samples[1]].startPoint;
				samplePoints[2] = Lc[samples[1]].endPoint;
				samplePoints[3] = globalOctree.at(samples[0]).normal;
				samplePoints[4] = Lc[samples[1]].normalsVec.front();
				samplePoints[5] = Lc[samples[1]].normalsVec.back();
			}
			else {
				samplePoints[0] = Lc[samples[0]].startPoint;
				samplePoints[1] = Lc[samples[0]].endPoint;
				samplePoints[2] = (Lc[samples[1]].startPoint + Lc[samples[1]].endPoint) / 2;
				samplePoints[3] = Lc[samples[0]].normalsVec.front();
				samplePoints[4] = Lc[samples[0]].normalsVec.back();
				samplePoints[5] = Lc[samples[1]].normals;
			}

			if (!(shape = (*planeShapeIter)->Construct(samplePoints)))
				continue;
#ifdef TEST
			if (samplePlan == 0) {
				Plan0SampleNum++;
			...
			for (size_t i = 0; i < 3; ++i)
			{
				shape->DistanceAndNormalDeviation(samplePoints[i], samplePoints[i + 3], &dn);
			...
			if (samplePlan == 2) {
				if (shape->Distance(Lc[samples[1]].startPoint) > 0.1 * m_options.m_epsilon) {
					verified = false;
				}
				if (shape->Distance(Lc[samples[1]].endPoint) > 0.1 * m_options.m_epsilon) {
					verified = false;
				}
			}
			...
			Candidate cand(shape, node->Level(), samplePlan);
...
#pragma omp critical
				{
					...
					/////////////////////////////////////////////////////////////////////
					(*samplePlanScores)[samplePlan].first += cand.ExpectedValue();
					++(*samplePlanScores)[samplePlan].second;
					////////////////////////////////////////////////////////////////////
...
bool RansacShapeDetector::FindBestCandidate(CandidatesType& candidates,
	...
	float* maxForgottenCandidate, float* candidateFailProb,
	MiscLib::Vector< double > samplePlanProbability) const
{
	...
	{
		if (CandidateFailureProbability(candidates[i].ExpectedValue(), currentSize - numInvalid, drawnCandidates, numLevels, samplePlanProbability) > m_options.m_probability)
			break;
	...
	while (candHeap.size())
	{
		...
		bestCandidateFailureProbability = CandidateFailureProbability(
			trial->ExpectedValue(),
			currentSize - numInvalid, drawnCandidates, numLevels, samplePlanProbability);
...
size_t
RansacShapeDetector::Detect(PointCloud& pc, LineCloud& Lc, size_t beginIdx, size_t endIdx,
	MiscLib::Vector< std::pair< RefCountPtr< PrimitiveShape >, size_t > >* shapes)
{
	...
	globalOctree.TraverseOctree();

	//globalOctree.visualization(*globalOctree.Root(), 0, 5);
	...
	/////////////////////////////////////////////////////////////////////////////////
	MiscLib::Vector< double > samplePlanProbability(3);
	for (size_t i = 0; i < samplePlanProbability.size(); ++i)
		samplePlanProbability[i] = 1.0 / samplePlanProbability.size();
	/////////////////////////////////////////////////////////////////////////////////
	size_t drawnCandidates = 0; // keep track of number of candidates
	// that have been generated
	size_t generatedCandidates = 0, testedCandidates = 0, tddFailCandidates = 0;
	...
	size_t numShapes = 0; // number of shapes found since the last housekeeping
	size_t numInvalid = 0; // number of points that have been assigned to a shape since the last housekeeping
	...
	do
	{
		...
		MiscLib::Vector< std::pair< float, size_t > > samplePlanScores(3);
		for (size_t i = 0; i < samplePlanScores.size(); ++i)
			samplePlanScores[i] = std::make_pair(0.f, 0u);
		...
		///////////////////////////////////////////////////////////////////////
		MiscLib::Vector< double >samplePlanProbSum(samplePlanProbability.size());
		samplePlanProbSum[0] = samplePlanProbability[0];
		for (size_t i = 1; i < samplePlanProbSum.size() - 1; ++i)
			samplePlanProbSum[i] = samplePlanProbability[i] +
			samplePlanProbSum[i - 1];
		samplePlanProbSum[samplePlanProbSum.size() - 1] = 1;
		////////////////////////////////////////////////////////////////////////
		// generate candidates
		...
		while (CandidateFailureProbability(bestExpectedValue,
			...
			globalOctTreeMaxNodeDepth, samplePlanProbability) > m_options.m_probability
		...
		while (FindBestCandidate(candidates, octrees, pc, subsetScoreVisitor,
			...
			&bestCandidateFailureProbability, samplePlanProbability))
		{
			if (!foundCandidate)
			{
				...
				for (size_t i = 0; i < candidates.size(); ++i)
				{
					if (candidates[i].ExpectedValue() * 1.4f > candidates.back().ExpectedValue())
					{
						size_t candPlan = candidates[i].Plan();
						//++samplePlanScores[candPlan].first;
						samplePlanScores[candPlan].first += candidates[i].ExpectedValue();
						++samplePlanScores[candPlan].second;
					}
				}
				UpdateLevelWeights(0.5f, samplePlanScores, &samplePlanProbability);
				////////////////////////////////////////////////////////////////////////////////
			}
			foundCandidate = true;
...
#pragma parallel for schedule(static, 100)
		...
		drawnCandidates, globalOctTreeMaxNodeDepth, samplePlanProbability) > m_options.m_probability
	...
	size_t samplePlan) const
{
	for (size_t tries = 0; tries < m_maxCandTries; tries++) {
		...
		*node = oct.NodeContainingPoint(oct.at(first), depth, &nodeRange);

		if (samplePlan == 0/* || samplePlan == 2*/) {
			samples->push_back(first);
			if ((*node)->pointIdx.size() < 3)
				continue;
			while (samples->size() < 3)
			{
				...
				do
				{
					i = (*node)->pointIdx[rn_rand() % (*node)->pointIdx.size()];
			...
			samples->push_back(first);
			if ((*node)->lineIdx.size() < 1)
				continue;
			samples->push_back((*node)->lineIdx[rn_rand() % (*node)->lineIdx.size()]);
			return true;
		}
		else {
			if ((*node)->lineIdx.size() < 2)
				continue;
			while (samples->size() < 2)
			{
				...
				do
				{
					i = (*node)->lineIdx[rn_rand() % (*node)->lineIdx.size()];
				} while ((std::find(samples->begin(), samples->end(), i) != samples->end()) && iter++ < 40);
				if (iter >= 40)
					break;
				samples->push_back(i);
			}
